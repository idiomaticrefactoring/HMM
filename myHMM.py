    ...
    def HMMfwd(self, a, b, o, pi):
        ...
        for t in xrange(1,T):
            for i in xrange(N):
                ...
                """
                alpha[i,t]=b[i,o[t]]*np.dot(alpha[:,t-1],a[:,i].T)
#        1        print("test:",np.dot(alpha[:,t-1],a[:,i].T))
#                return 0
        return alpha 
    ...
    def HMMbwd(self, a, b, o):
        ...
        for t in xrange(T-2,-1,-1):
            for i in xrange(N):
                ...
                """
#                c=
#                print("test:",type(b),b.shape,a[i],b.T,o[t+1])
#                print(a[i]*b.T[o[t+1]])
                beta[i,t]=beta[:,t+1].dot(a[i]*b.T[o[t+1]])
#                print(beta[:,t+1],a[i]*b.T[o[t+1]],beta[:,t+1].dot(a[i]*b.T[o[t+1]]))
#                return 0
        return beta
    ...
    def HMMViterbi(self, a, b, o, pi):
        ...
        for t in range(1,T):  
             for i in range(0,N):                                 
                 delta[i,t] = b[i,o[t]]*np.array([delta[j,t-1]*a[j,i]  for j in range(N)]).max()
                 phi[i,t] =np.array([delta[j,t-1]*a[j,i]  for j in range(N)]).argmax()
...
if __name__ == '__main__':
    ...
    print("hist_Observation:",hist_Observation)
